...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!isset($_SESSION['user_id'])) {
        ...
        header("Location: login.php?redirect=computer.php");
        exit();
    }

    $monitor_no = $_POST['monitor_no'];
    ...
    $sql = "INSERT INTO computer (monitor_no, department, location, name, branch, prn_no, complaint_description) VALUES (?, ?, ?, ?, ?, ?, ?)";
    $stmt = $conn->prepare($sql);
    if ($stmt) {
        $stmt->bind_param("sssssss", $monitor_no, $department, $location, $name, $branch, $prn_no, $complaint_description);
        if ($stmt->execute()) {
...
<nav class="navbar">
    ...
    <div class="login-container">
        <div class="form-container">
            ...
            <?php endif; ?>
            <form class="login-form" action="computer.php" method="POST" onsubmit="return checkLoginStatus()">
                <h2>Monitor Details</h2>
                <div class="input-group">
                    <label for="monitor_no">Monitor No:</label>
                    <input type="text" id="monitor_no" name="monitor_no" required>
                </div>
                <div class="input-group">
    ...
    <script>
        function checkLoginStatus() {
            <?php if (!isset($_SESSION['user_id'])): ?>
                ...
                window.location.href = "login.php?redirect=computer.php";
                return false;
            <?php endif; ?>
            return true;
        }
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (qrData) {
                try {
                    ...
                    document.getElementById('monitor_no').value = decodedData['Monitor no'] || '';
